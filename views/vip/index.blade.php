...
<ul class="breadcrumb">
    ...
    <li class="active">รายการสมาชิกวีไอพี</li>
</ul>
@stop


@section('pageheader')
<h1>รายการสมาชิกวีไอพี</h1>
@stop
...
{{ Form::open( array( 'url'=>'vip/delall', 'id'=>'form-del-delall') ) }}   
    ...
    <div class="col-xs-12">
        ...
        <div>
            <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                ...
                <tbody>
                    <?php $i = 1; ?>
                    @foreach( $result as $key => $vip )
                    <tr>
                        <td class="center"><label class="pos-rel"><input id="{{ 'chkbox-'.$i }}" type="checkbox" class="ace dataTableChkbox" value="{{ $vip->mb_mem_id }}" /><span class="lbl"></span></label></td>
                        <td class="center hidden-480">{{ $i }}</td>
                        <td class="center">{{ HTML::Link('vip/'.$vip->mb_mem_id, $vip->mb_mem_code) }}</td>
                        <td class="hidden-480">{{ $vip->mb_mem_fnameth.' '.$vip->mb_mem_lnameth }}</td>
                        <td class="hidden-480">{{ $vip->mb_mem_email }}</td>
                        <td class="center">{{ GetFormat::format_DateTime($vip->mb_mem_sdate) }}</td>
                        <td class="center">{{ GetList::$list_type[$vip->mb_mem_type] }}</td>
                        <td class="center">
                            <div class="hidden-sm hidden-xs action-buttons">   
                                {{ HTML::decode(link_to('vip/'.$vip->mb_mem_id.'/edit','<i class="ace-icon fa fa-pencil bigger-130"></i>',array('class'=>'green'))) }}

                                <a href='javascript:void(0)' class="red bootbox-confirm" onclick="
                                    document.getElementById('hidden_rowid').value='{{ $vip->mb_mem_id }}';
                                    document.getElementById('hidden_rowcode').value='{{ $vip->mb_mem_code }}';
                                "><i class="ace-icon fa fa-trash-o bigger-130"></i></a>
                                {{ Form::open( array( 'url'=>'vip/'.$vip->mb_mem_id, 'id'=>'form-del-'.$vip->mb_mem_id ) )}}
                                {{ Form::hidden( '_method','DELETE' ) }}
                                {{ Form::close() }}
