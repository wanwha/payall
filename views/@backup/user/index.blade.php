...
{{ Form::open( array( 'url'=>'user/delall', 'id'=>'form-del-delall') ) }}   
...
<div class="row">
    <div class="col-xs-12">
        ...
        <div class="clearfix">
            <a class="btn btn-sm btn-primary" href="{{ URL::to('user/create') }}">สร้างใหม่</a>
            ...
            <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                <thead>
                    <tr>
                        ...
                        <th class="center hidden-480">ชื่อ-นามสกุล</th>          
                        <th class="center hidden-480">วันที่แก้ไข</th>
                        <th class="center hidden-480">สิทธิการใช้งาน</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="center"><label class="pos-rel"><input id="{{ 'chkbox-'.$i }}" type="checkbox" class="ace dataTableChkbox" value="{{ $user->id }}" /><span class="lbl"></span></label></td>
                        <td class="center hidden-480">{{ $i }}</td>
                        <td class="">{{ HTML::link('user/'.$user->id,$user->email) }}</td>
                        <td class="hidden-480">{{ $user->first_name.' '.$user->last_name}}</td>    
                        <td class="center hidden-480">{{ EditFormat::format_DateTime($user->updated_at) }}</td>
                        <td class="center hidden-480">{{ $list_group[$user->group_id] }}</td>
         
                        <td class="center">
                            <div class="hidden-sm hidden-xs action-buttons"> 
                            
                                {{ HTML::decode(link_to('user/'.$user->id.'/edit','<i class="ace-icon fa fa-pencil bigger-130"></i>',array('class'=>'green'))) }}
                               <a href='javascript:void(0)' class="red bootbox-confirm" onclick=" document.getElementById('hidden_rowid').value='{{ $user->id }}';document.getElementById('hidden_rowcode').value='{{ $user->id }}';"><i class="ace-icon fa fa-trash-o bigger-130"></i></a>
                                {{ Form::open( array( 'url'=>'user/'.$user->id, 'id'=>'form-del-'.$user->id ) )}}
                                {{ Form::hidden( '_method','DELETE' ) }}
                                {{ Form::close() }}
                            </div>
                            ...
                            <div class="hidden-md hidden-lg">
                                <div class="inline pos-rel">
                                    ...
                                    <ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
                                        <li>{{ HTML::decode(link_to('user/'.$user->id.'/edit', '<span class="green"><i class="ace-icon fa fa-pencil-square-o bigger-120"></i></span>', array('class'=>'tooltip-success','data-rel'=>'tooltip','title'=>'Edit'))) }}</li>                  
                                        <li><a href='javascript:void(0)' class="red tooltip-error bootbox-confirm" data-rel='tooltip' title='Delete' onclick=" document.getElementById('hidden_rowid').value='{{ $user->id }}';document.getElementById('hidden_rowcode').value='{{ $user->id }}';"><i class="ace-icon fa fa-trash-o bigger-130"></i></a>
                                        {{ Form::open( array( 'url'=>'user/'.$user->id, 'id'=>'form-del-'.$user->id ) )}}
                                        {{ Form::hidden( '_method','DELETE' ) }}
                                        {{ Form::close() }}</li>
                                    </ul>
...
jQuery(function ($) {
        ...
        $(".bootbox-confirm").on(ace.click_event, function () {
            ...
            bootbox.confirm({
                message: "ต้องการลบผู้ใช้ รหัส "+row_code+" ถูกต้องใช่หรือไม่?",
                buttons: {
                    confirm: {
                        label: "ลบ",
 ...
 jQuery(function ($) {
        ...
        $(".bootbox-confirm-delall").on(ace.click_event, function () {
            ...
            bootbox.confirm({
                message: "ต้องการลบผู้ใช้ที่เลือกทั้งหมด ใช่หรือไม่?",
                buttons: {
                    confirm: {
                        label: "ลบ",
                        className: "btn-danger btn-sm",
